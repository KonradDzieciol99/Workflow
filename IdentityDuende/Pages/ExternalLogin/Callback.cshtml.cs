...
public class Callback : PageModel
{
    ...
    private async Task<ApplicationUser> AutoProvisionUserAsync(string provider, string providerUserId, IEnumerable<Claim> claims)
    {
        ...
        var user = new ApplicationUser
        {
            ...
            PictureUrl = claims.FirstOrDefault(x => x.Type == JwtClaimTypes.Picture)?.Value ?? null,
            Email = email,
        };
    ...
    private async Task UpdateUserPictureUrlIfNewClaimPictureExists(ApplicationUser user, IEnumerable<Claim> claims)
    {
        ...
        {
            if (user.PictureUrl == null || !user.PictureUrl.Equals(pictureClaim))
            {
                user.PictureUrl = pictureClaim;
                var identityResult = await _userManager.UpdateAsync(user);
                if (!identityResult.Succeeded)
                    throw new Exception("Failed to update user picture URL.");
            }
        }
    }
}