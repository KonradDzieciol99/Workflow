...
public class AppNotificationRepository : Repository<AppNotification>, IAppNotificationRepository
{
    ...
    public AppNotificationRepository(ApplicationDbContext applicationDbContext)
    ...
    public async Task<bool> CheckIfUserIsAOwnerOfAppNotification(
        string appNotificationId,
        string userId
    )
    {
        return await _applicationDbContext.AppNotification.AnyAsync(
            x => x.Id == appNotificationId && x.UserId == userId
        );
    }
    ...
    public async Task<List<AppNotification>> GetByNotificationPartnersIdsAsync(
        string NotificationPartnerIdOne,
        string NotificationPartnerIdTwo,
        List<NotificationType> notificationTypes
    )
    {
        return await _applicationDbContext.AppNotification
            .Where(
                x =>
                    (
                        (
                            x.UserId == NotificationPartnerIdOne
                            && x.NotificationPartnerId == NotificationPartnerIdTwo
                        )
                        || (
                            x.UserId == NotificationPartnerIdTwo
                            && x.NotificationPartnerId == NotificationPartnerIdOne
                        )
    ...
    public async Task<(List<AppNotification> AppNotifications, int totalCount)> GetAsync(
        string userId,
    ...
    )
    {
        ...
        var projects = await query.Skip(@params.Skip).Take(@params.Take).ToListAsync();
    ...
    {
        return await _applicationDbContext.AppNotification
            .Where(n => n.UserId == userId && n.Displayed == false)
            .Select(n => n.Id)
            .ToListAsync();
    }
}
