...
public record FriendRequestAcceptedEvent(string InvitationSendingUserId, string InvitationSendingUserEmail, string? InvitationSendingUserPhotoUrl, string InvitationAcceptingUserId, string InvitationAcceptingUserEmail, string? InvitationAcceptingUserPhotoUrl) : IntegrationEvent;
public class FriendRequestAcceptedEventHandler : IRequestHandler<FriendRequestAcceptedEvent>
{
    ...
    public async Task Handle(FriendRequestAcceptedEvent request, CancellationToken cancellationToken)
    {
        ...
        await _messagesHubContext.Clients.User(request.InvitationSendingUserId).SendAsync("FriendInvitationAccepted", friendInvitationDto, cancellationToken: cancellationToken);

        await _messagesHubContext.Clients.User(request.InvitationAcceptingUserId).SendAsync("FriendInvitationAccepted", friendInvitationDto, cancellationToken: cancellationToken);

        var isOnline = await _redisDb.KeyExistsAsync($"presence-{request.InvitationSendingUserEmail}");

        if (isOnline)
            await _presenceHubContext.Clients.User(request.InvitationAcceptingUserId).SendAsync("UserIsOnline", request.InvitationSendingUserEmail, cancellationToken: cancellationToken);

        isOnline = await _redisDb.KeyExistsAsync($"presence-{request.InvitationAcceptingUserEmail}");
        if (isOnline)
            await _presenceHubContext.Clients.User(request.InvitationSendingUserId).SendAsync("UserIsOnline", request.InvitationAcceptingUserEmail, cancellationToken: cancellationToken);
