...
public record GetConfirmedFriendRequestsQuery(int Skip, int Take, string? Search)
    : IAuthorizationRequest<List<FriendRequestDto>>
{
    public List<IAuthorizationRequirement> GetAuthorizationRequirement() => new();
}

public class GetConfirmedFriendRequestsQueryHandler
    : IRequestHandler<GetConfirmedFriendRequestsQuery, List<FriendRequestDto>>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly ICurrentUserService _currentUserService;
    private readonly IMapper _mapper;

    public GetConfirmedFriendRequestsQueryHandler(
    ...
    public async Task<List<FriendRequestDto>> Handle(
        GetConfirmedFriendRequestsQuery request,
        CancellationToken cancellationToken
    )
    {
        var friendRequests = await _unitOfWork.FriendRequestRepository.GetConfirmedAsync(
