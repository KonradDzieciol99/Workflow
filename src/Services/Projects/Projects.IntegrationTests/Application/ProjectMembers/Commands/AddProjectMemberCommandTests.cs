...
public class AddProjectMemberCommandTests : IAsyncLifetime
{
    ...
    public async Task AddProjectMemberCommand_ValidData_ReturnsProjectMemberDto()
    {
        ...
        var response = await _base._client.PostAsync(
            $"api/Projects/{command.ProjectId}/projectMembers/addMember",
        ...
        var returnedProjectMemberDto = JsonSerializer.Deserialize<ProjectMemberDto>(
            responseString,
            options
        );
