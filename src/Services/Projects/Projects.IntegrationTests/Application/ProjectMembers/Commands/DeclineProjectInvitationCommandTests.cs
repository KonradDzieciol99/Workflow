...
public class DeclineProjectInvitationCommandTests: IAsyncLifetime
{
    ...
    public async Task DeclineProjectInvitationCommand_ValidData_ReturnsNoContent()
    {
        ...
        var memberWhoDeclined = new ProjectMember("testUserId2", "testUserEmail@test.com2", null, ProjectMemberType.Leader, InvitationStatus.Invited);
        ...
        //act
        var response = await _base._client.DeleteAsync($"api/Projects/{command.ProjectId}/DeclineInvitation");

        //assert
        var invitedMemverAfterdeclined = await _base._factory.FindAsync<Program, ApplicationDbContext, ProjectMember>(memberWhoDeclined.Id);
