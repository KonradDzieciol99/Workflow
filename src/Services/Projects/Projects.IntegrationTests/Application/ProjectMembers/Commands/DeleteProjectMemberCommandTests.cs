...
public class DeleteProjectMemberCommandTests: IAsyncLifetime
{
    ...
    public async Task DeclineProjectInvitationCommand_ValidData_ReturnsNoContent()
    {
        ...
        //act
        var response = await _base._client.DeleteAsync($"api/Projects/{command.ProjectId}/projectMembers/{invitedMember.Id}");

        //assert
        var invitedMemverAfterdeclined = await _base._factory.FindAsync<Program, ApplicationDbContext, ProjectMember>(invitedMember.Id);
