...
public class UpdateProjectMemberCommandTests: IAsyncLifetime
{
    ...
    public async Task UpdateProjectMemberCommand_ValidData_ReturnsNoContent()
    {
        ...
        var invitedMember = new ProjectMember("testUserId2", "testUserEmail@test.com2", null, ProjectMemberType.Member, InvitationStatus.Accepted);
        ...
        UpdateProjectMemberCommand command = new(ProjectMemberType.Admin, projects[0].Id, invitedMember.UserId);

        //act
        var response = await _base._client.PutAsync($"api/Projects/{command.ProjectId}/projectMembers/{invitedMember.UserId}", command.ToStringContent());

        //assert
        var invitedMemverAfterAction = await _base._factory.FindAsync<Program, ApplicationDbContext, ProjectMember>(invitedMember.Id);
