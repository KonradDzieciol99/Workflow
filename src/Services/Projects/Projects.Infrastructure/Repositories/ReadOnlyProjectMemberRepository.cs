...
public class ReadOnlyProjectMemberRepository : IReadOnlyProjectMemberRepository
{
    ...
    {
        return await ProjectMembersQuery
            .Include(pm => pm.MotherProject)
    ...
    {
        return await ProjectMembersQuery.SingleOrDefaultAsync(
            x => x.UserId == userId && x.MotherProject.Id == projectId
        );
    }

    public async Task<(List<Project> Projects, int TotalCount)> Get(
        string userId,
        GetProjectsQuery appParams
    )
    {
        ...
        query = string.IsNullOrWhiteSpace(appParams.Search) switch
        {
            ...
            false
                => query.Where(
                    x => x.UserId == userId && x.MotherProject.Name.StartsWith(appParams.Search)
                ),
        };
    ...
    {
        return await ProjectMembersQuery.AnyAsync(
    ...
    public async Task<bool> CheckIfUserIsALeaderAsync(string projectId, string userId)
    {
        return await ProjectMembersQuery.AnyAsync(
            x =>
                x.UserId == userId && x.ProjectId == projectId && x.Type == ProjectMemberType.Leader
        );
    }
    ...
    public async Task<int> RemoveAsync(string projectId, string userId)
    {
        return await ProjectMembersQuery
            .Where(
                x =>
                    x.UserId == userId
                    && x.MotherProject.Id == projectId
                    && x.Type != ProjectMemberType.Leader
            )
            .ExecuteDeleteAsync();
    }

    public async Task<List<MemberStatusDto>> GetMembersStatusesAsync(
        string projectId,
        List<string> usersIds
    )
    {
        var memberStatuses = await ProjectMembersQuery
            .Where(x => x.ProjectId == projectId && usersIds.Contains(x.UserId))
            .Select(
                m =>
                    new MemberStatusDto
                    {
                        UserId = m.UserId,
                        Status =
                            m.InvitationStatus == InvitationStatus.Invited
                                ? MemberStatusType.Invited
                                : MemberStatusType.Member,
                    }
            )
            .ToListAsync();
        ...
        return usersIds
            .Select(
                id =>
                    memberStatuses.FirstOrDefault(s => s.UserId == id)
