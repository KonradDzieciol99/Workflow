...
public class GetAppTasksQueryTests : IAsyncLifetime //to może zastępowac konstruktor i dispose (bardziej to przejrzyste)
{
    ...
    public static IEnumerable<object[]> GetAppTasksQueryList =>
        new List<object[]>
        {
            ...
            new object[]
            {
                new GetAppTasksQuery("1", 0, 10, null, null, null, null, "task3", null)
            },
        };

    [Theory]
    [MemberData(nameof(GetAppTasksQueryList))]
    public async Task GetAppTasksQuery_WithValidRouteParamsAndQueryParams_ReturnsAppTaskList(
        GetAppTasksQuery query
    )
    {
        ...
        var response = await _base._client.GetAsync(
            $"api/projects/{projectMembers[0].ProjectId}/task?{queryParams}"
        );
    ...
    public async Task GetAppTasksQuery_WithInvalidRouteParamsAndInvaliQueryParams_ReturnsForbidden()
    {
        ...
        //act
        var response = await _base._client.GetAsync(
            $"api/projects/{fakeQuery.ProjectId}/task?{fakeQuery.ToQueryString()}"
        );
