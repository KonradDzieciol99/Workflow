...
public class DeleteAppTaskCommandTests : IAsyncLifetime //to może zastępowac konstruktor i dispose (bardziej to przejrzyste)
{
    ...
    public async Task DeleteAppTaskCommand_ValidData_ReturnsAppTask()
    {
        ...
        var response = await _base._client.DeleteAsync(
            $"api/projects/{command.ProjectId}/task/{command.Id}"
        );

        //assert

        var item = await _base._factory.FindAsync<Program, ApplicationDbContext, AppTask>(
            addedAppTasks[0].Id
        );
    ...
    public async Task DeleteAppTaskCommand_InValidData_ReturnsAppTask()
    {
        ...
        //act
        var response = await _base._client.DeleteAsync(
            $"api/projects/{projectMembers[0].ProjectId}/task/{fakeTaskId}"
        );
