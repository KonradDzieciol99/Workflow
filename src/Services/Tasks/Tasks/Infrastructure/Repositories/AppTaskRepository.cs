...
public class AppTaskRepository : Repository<AppTask>, IAppTaskRepository
{
    ...
    public AppTaskRepository(ApplicationDbContext applicationDbContext)
        : base(applicationDbContext)
    {
        _applicationDbContext = applicationDbContext;
    }

    public async Task<bool> CheckIfUserHasRightsToMenageTaskAsync(
        string projectId,
        string userId,
        string appTaskId
    )
    {
        return await _applicationDbContext.ProjectMembers.AnyAsync(
            x =>
                x.UserId == userId
    ...
    {
        return await _applicationDbContext.AppTasks
    ...
    public async Task<(List<AppTask> appTasks, int totalCount)> GetAsync(
        string userId,
        GetAppTasksQuery @params
    )
    {
        var query = _applicationDbContext.AppTasks
            .Include(t => t.TaskLeader)
            .Include(t => t.TaskAssignee)
            .AsQueryable();
        ...
        query = string.IsNullOrWhiteSpace(@params.Search) switch
        {
            ...
            false
                => query.Where(
                    x => x.ProjectId == @params.ProjectId && x.Name.StartsWith(@params.Search)
                ),
        };
