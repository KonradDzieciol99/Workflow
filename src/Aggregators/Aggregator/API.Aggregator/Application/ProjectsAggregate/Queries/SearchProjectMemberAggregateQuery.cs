...
public record SearchProjectMemberAggregateQuery(string Email, string ProjectId, int Take, int Skip)
    : IRequest<List<SearchedMemberDto>> { }

public class SearchMemberQueryHandler
    : IRequestHandler<SearchProjectMemberAggregateQuery, List<SearchedMemberDto>>
{
    private readonly IProjectsService _projectsService;
    private readonly IIdentityServerService _identityServerService;

    public SearchMemberQueryHandler(
    ...
    public async Task<List<SearchedMemberDto>> Handle(
        SearchProjectMemberAggregateQuery request,
    ...
    )
    {
        ...
        var SearchedMembers = usersFound
            .Select(
                (x, index) => new SearchedMemberDto(x.Id, x.Email, x.PhotoUrl, status[index].Status)
            )
            .ToList();
