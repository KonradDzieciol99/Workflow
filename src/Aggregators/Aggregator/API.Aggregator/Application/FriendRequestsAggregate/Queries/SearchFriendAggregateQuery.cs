...
public record SearchFriendAggregateQuery(string Email, int Take, int Skip) : IRequest<List<SearchedUserDto>>;

public class SearchFriendAggregateQueryHandler : IRequestHandler<SearchFriendAggregateQuery, List<SearchedUserDto>>
{
    private readonly IIdentityServerService _identityServerService;
    private readonly IChatService _chatService;

    public SearchFriendAggregateQueryHandler(IIdentityServerService identityServerService, IChatService chatService)
    {
    ...
    public async Task<List<SearchedUserDto>> Handle(SearchFriendAggregateQuery request, CancellationToken cancellationToken)
    {
        ...
        return usersFound.Select((x, index) => new SearchedUserDto(x.Id, x.Email, x.PhotoUrl, status[index].Status)).ToList();
    }
}
